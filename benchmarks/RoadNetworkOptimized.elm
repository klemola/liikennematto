module RoadNetworkOptimized exposing
    ( CellIndex
    , Connection
...
completeLane tilemapConfig spatialIndex current =
    let
        ...
        nearbyNodes =
            getNearbyNodes tilemapConfig spatialIndex current.label.cell
    in
    nearbyNodes
...
getNearbyNodes : TilemapConfig -> CellIndex -> Cell -> List (Node Connection)
getNearbyNodes tilemapConfig spatialIndex cell =
    List.concatMap
        (\( _, nearbyCell ) ->
            Dict.get (Cell.coordinates nearbyCell) spatialIndex
                |> Maybe.withDefault []
        )
        (( OrthogonalDirection.Up, cell )
...
--
-- Traffic control (unchanged from original)
--
