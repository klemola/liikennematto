    ...
    describe "World"
        [ describe "World.isEmptyArea"
            [ test "correctly determines if an area is empty (not existings lots)"
                (\_ ->
                    World.isEmptyArea { origin = ( 80, 160 ), width = 160, height = 160 } Fixtures.worldThatHasAVerticalRoadAtLeftSide
            ...
            , test "correctly determines if an area is empty (not enough space between roads)"
                (\_ ->
                    World.isEmptyArea { origin = ( 80, 160 ), width = 160, height = 160 } Fixtures.worldThatHasParallelRoads
                        |> Expect.false "Expected the \"world\" *not* to have space."
                )
            , test "reports area as filled if it's out of board bounds"
                (\_ ->
                    World.isEmptyArea { origin = ( 720, 80 ), width = 240, height = 160 } Fixtures.worldThatHasAVerticalRoadAtLeftSide
                        |> Expect.false "Expected the \"world\" *not* to have space."
                )
            ]
